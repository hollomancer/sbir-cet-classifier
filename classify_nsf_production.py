#!/usr/bin/env python
"""Production classification of all NSF SBIR awards with enrichment and reporting."""
...
def load_nsf_awards(csv_path: Path, limit: int = None):
    """Load NSF awards from bootstrap CSV."""
    print(f"=== Loading Awards from {csv_path} ===")
    
    result = load_bootstrap_csv(csv_path)
    
    # Filter to NSF awards only
    nsf_awards = [
        award for award in result.awards
        if award.agency == 'NSF' or 'science' in award.agency.lower() and 'foundation' in award.agency.lower()
    ]
    
    if limit:
        nsf_awards = nsf_awards[:limit]
    
    print(f"✓ Loaded {len(nsf_awards)} NSF awards (from {len(result.awards)} total)")
    print(f"  Total rows: {result.total_rows}")
    print(f"  Loaded: {result.loaded_count}")
    print(f"  Skipped: {result.skipped_count}")
    
    return nsf_awards, result


def enrich_awards_batch(awards, cache, client, use_api=False):
    """Enrich awards with NSF API or fallback."""
    ...
    for i, award in enumerate(awards):
        ...
        # For now, use fallback enrichment (NSF API integration can be added later)
...
def train_classifier(enriched_awards, sample_size=1000):
    ...
    cet_keywords = {
        ...
        'semiconductors': ['semiconductor', 'chip', 'microelectronics', 'silicon'],
    }
    ...
    for award, text in training_sample:
        ...
        cet_id = max(scores, key=scores.get) if max(scores.values()) > 0 else 'artificial_intelligence'
...
def generate_report(results, enrichment_stats, classification_time, output_dir: Path):
    ...
    results_path = output_dir / f"nsf_classification_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    # Save JSON report
    report_path = output_dir / f"nsf_classification_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"NSF SBIR CLASSIFICATION REPORT")
    print(f"{'='*60}")
    print(f"\nTotal Awards: {len(df):,}")
...
def main():
    """Run production NSF classification."""
    print("="*60)
    print("NSF SBIR PRODUCTION CLASSIFICATION")
    print("="*60)
    ...
    use_api = False  # Set to True to enable NSF API calls
    
    # Load awards
    awards, load_result = load_nsf_awards(csv_path, limit=limit)
    
    if not awards:
        print("✗ No NSF awards found")
        return
    
    # Initialize enrichment
    cache = SolicitationCache()
    client = NSFClient() if use_api else None
