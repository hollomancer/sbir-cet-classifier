...
class CETAwareTfidfVectorizer:
    ...
    def __init__(
        ...
        **tfidf_params,
        ...
        # CET keyword mappings from YAML-backed configuration (core + related terms)
        try:
            from sbir_cet_classifier.common.classification_config import get_cet_keywords_map

            keyword_models = get_cet_keywords_map()
            self.cet_keywords_set: Set[str] = set()
            for _, buckets in keyword_models.items():
                core = getattr(buckets, "core", []) or []
                related = getattr(buckets, "related", []) or []
                for kw in list(core) + list(related):
                    if kw:
                        self.cet_keywords_set.add(str(kw).lower())
        except Exception:
            # Fallback: use any custom vocabulary provided, otherwise empty
            self.cet_keywords_set = set(kw.lower() for kw in (self.custom_cet_vocabulary or []))
    ...
    def get_cet_keywords(self) -> Set[str]:
        ...
        # Prefer YAML-backed keywords if available, fall back to custom vocabulary
        if hasattr(self, "cet_keywords_set") and self.cet_keywords_set:
            return set(self.cet_keywords_set)
        return set(self.custom_cet_vocabulary or [])
...
class SolicitationEnhancedTfidfVectorizer:
    ...
    def _build_combined_vocabulary(self):
        ...
        abstract_features = [
            f"abstract_{name}" for name in self.abstract_vectorizer.get_feature_names_out()
        ]
        keywords_features = [
            f"keywords_{name}" for name in self.keywords_vectorizer.get_feature_names_out()
        ]
        solicitation_features = [
            f"solicitation_{name}" for name in self.solicitation_vectorizer.get_feature_names_out()
        ]
