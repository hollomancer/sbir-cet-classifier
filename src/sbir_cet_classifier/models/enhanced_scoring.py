...
class MultiSourceTextVectorizer:
    ...
    def __init__(self, abstract_weight: float = 0.5, keywords_weight: float = 0.2, 
                 solicitation_weight: float = 0.3, **tfidf_params):
    ...
    def get_feature_names_out(self) -> List[str]:
        ...
        abstract_names = [f"abstract_{name}" for name in self.abstract_vectorizer_.get_feature_names_out()]
        keywords_names = [f"keywords_{name}" for name in self.keywords_vectorizer_.get_feature_names_out()]
        solicitation_names = [f"solicitation_{name}" for name in self.solicitation_vectorizer_.get_feature_names_out()]
...
class EnhancedCETClassifier:
    ...
    def __init__(self, include_solicitation_text: bool = True,
    ...
    def prepare_training_data(self, awards_data: List[Dict[str, Any]]) -> Tuple[csr_matrix, List[str]]:
    ...
    def predict_top_categories(self, award_data: Dict[str, Any], top_n: int = 3) -> List[Tuple[str, float]]:
...
class SolicitationEnhancedScorer:
    ...
    def enhance_award_with_solicitation(self, award: Dict[str, Any], 
                                      solicitation: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def combine_scores_with_solicitation_boost(self, base_scores: Dict[str, float],
                                             solicitation_scores: Dict[str, float],
                                             boost_factor: float = 0.3) -> Dict[str, float]:
    ...
    def batch_enhance_awards(self, awards: List[Dict[str, Any]], 
                           solicitations: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
