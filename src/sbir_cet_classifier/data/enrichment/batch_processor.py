...
class SolicitationBatchProcessor:
    ...
    async def process_batch(self, awards: List[Dict[str, Any]], 
                          skip_existing: bool = False,
                          save_to_storage: bool = False,
                          max_retries: int = 0,
                          progress_callback: Optional[Callable[[int, int, str], None]] = None) -> BatchProcessingResults:
        ...
        for i in range(0, len(awards), self.batch_size):
            ...
            batch_results = await self._process_batch_chunk(
                batch, skip_existing, max_retries, progress_callback, 
                i, len(awards)
            )
        ...
        if save_to_storage and self.storage and successful:
            try:
                solicitations_to_save = []
                for solicitation_id in successful:
                    # This would need to be implemented to retrieve the actual solicitation objects
                    pass
                
                if solicitations_to_save:
                    self.storage.save_solicitations(solicitations_to_save)
    ...
    async def _process_batch_chunk(self, batch: List[Dict[str, Any]], 
                                 skip_existing: bool,
                                 max_retries: int,
                                 progress_callback: Optional[Callable],
                                 batch_start_idx: int,
                                 total_awards: int) -> Dict[str, List]:
        """Process a single batch chunk with concurrency control."""
        semaphore = asyncio.Semaphore(self.max_concurrent)
        
        async def process_single_award(award: Dict[str, Any]) -> Tuple[str, Optional[str], Optional[Solicitation]]:
            ...
            async with semaphore:
                ...
                for attempt in range(max_retries + 1):
                    try:
                        solicitation = await self.solicitation_service.get_solicitation_by_number(solicitation_number)
                        if solicitation:
...
class BatchEnrichmentPipeline:
    ...
    async def enrich_awards_with_solicitations(self, awards: List[Dict[str, Any]],
                                             batch_size: int = 50,
    ...
    async def retry_failed_enrichments(self, failed_awards: List[Dict[str, Any]]) -> BatchProcessingResults:
