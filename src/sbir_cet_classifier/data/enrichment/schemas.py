...
class SAMAwardResponse(BaseModel):
    ...
    recipient_duns: Optional[str] = Field(None, alias="recipientDUNS")
...
class PhysicalAddress(BaseModel):
    ...
    city: Optional[str] = None
    state_or_province_code: Optional[str] = Field(None, alias="stateOrProvinceCode")
...
class Solicitation(BaseModel):
    ...
    def validate_funding_range(cls, v, values):
        ...
        if v is not None and 'funding_range_min' in values:
            min_funding = values['funding_range_min']
...
class AwardModification(BaseModel):
    ...
    funding_change: Optional[Decimal] = Field(None, description="Funding amount change (can be negative)")
