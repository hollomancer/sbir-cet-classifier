...
class AwardeeProfileValidator:
    ...
    def validate_funding_consistency(self, profile: AwardeeProfile) -> ValidationResult:
        ...
        if profile.total_awards > 0:
            ...
            if abs(expected_avg - actual_avg) > Decimal("1.00"):
                result.add_warning(
                    "avg_award_amount",
                    f"Funding consistency: average award amount ({actual_avg}) doesn't match calculated average ({expected_avg})",
                )
    ...
    def validate_date_consistency(self, profile: AwardeeProfile) -> ValidationResult:
        ...
        if profile.first_award_date > profile.last_award_date:
            result.add_error("first_award_date", "First award date cannot be after last award date")
    ...
    def validate_agency_code(self, agency: str) -> ValidationResult:
        """Validate a single agency code."""
        result = ValidationResult()
        if not agency:
            result.add_error("primary_agencies", "Agency code is required")
            return result
        if agency not in self.valid_agencies:
            result.add_error("primary_agencies", f"Invalid agency code: {agency}")
        return result
    ...
    def _validate_cross_fields(self, profile: AwardeeProfile) -> ValidationResult:
        ...
        if profile.success_rate > 0.9 and profile.avg_award_amount < Decimal("10000"):
            result.add_warning(
                "success_rate", "High success rate with very low average funding is unusual"
            )

        # Many awards with very high success rate might be suspicious
        if profile.total_awards > 50 and profile.success_rate > 0.95:
            result.add_warning("success_rate", "Very high success rate with many awards is unusual")
