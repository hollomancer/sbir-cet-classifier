...
class Solicitation(BaseModel):
    ...
    cet_relevance_scores: Dict[str, float] = Field(default_factory=dict, description="Pre-computed CET category relevance")
    created_at: datetime = Field(..., description="Record creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")
    
    @validator('funding_range_max')
    def validate_funding_range(cls, v, values):
        """Validate that funding_range_max >= funding_range_min."""
        if 'funding_range_min' in values and v < values['funding_range_min']:
            raise ValueError('funding_range_min must be <= funding_range_max')
        return v
    
    @validator('performance_period')
    ...
    @validator('cet_relevance_scores')
...
class ProgramOffice(BaseModel):
    ...
    strategic_focus_areas: List[str] = Field(default_factory=list, description="Key technology/research areas")
...
class AwardeeProfile(BaseModel):
    ...
    primary_agencies: List[str] = Field(default_factory=list, description="Primary funding agencies")
    technology_areas: List[str] = Field(default_factory=list, description="Primary technology areas")
    
    @validator('avg_award_amount')
    def validate_avg_award_amount(cls, v, values):
        """Validate that average award amount is consistent with totals."""
        if 'total_awards' in values and 'total_funding' in values:
...
class AwardeeMatchResult(BaseModel):
    ...
    confidence_score: float = Field(..., ge=0.0, le=1.0, description="Match confidence (0.0 to 1.0)")
    match_method: str = Field(..., description="Method used for matching")
    match_details: Dict[str, Any] = Field(default_factory=dict, description="Detailed match information")
...
class AwardeeMetrics(BaseModel):
    ...
    award_size_distribution: Optional[Dict[str, int]] = Field(None, description="Award size distribution")
...
class AwardeeEnrichmentService:
    ...
    def match_award_to_awardee(self, award_data: Dict[str, Any]) -> Optional[AwardeeMatchResult]:
        ...
        if awardee_name:
            ...
            if entities:
                ...
                confidence = self._calculate_name_similarity(awardee_name, best_match.get("legalBusinessName", ""))
    ...
    def enrich_awardee_profile(self, uei: str) -> Optional[AwardeeProfile]:
        ...
        for award in awards:
            ...
            if start_date_str:
                try:
                    award_dates.append(datetime.fromisoformat(start_date_str.replace('Z', '+00:00')))
                except:
                    pass
        ...
        primary_agencies = list(set(award.get("fundingAgency") for award in awards if award.get("fundingAgency")))
