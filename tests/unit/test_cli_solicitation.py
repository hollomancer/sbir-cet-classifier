...
class TestSolicitationCLI:
    ...
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.SolicitationService')
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.SAMClient')
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.SolicitationStorage')
    def test_enrich_solicitation_success(self, mock_storage_class, mock_client_class, 
                                       mock_service_class, mock_asyncio_run, runner):
    ...
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.SolicitationService')
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.SAMClient')
    def test_enrich_solicitation_not_found(self, mock_client_class, mock_service_class, 
                                         mock_asyncio_run, runner):
    ...
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.pd.read_csv')
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.asyncio.run')
    @patch('src.sbir_cet_classifier.cli.enrichment_commands.SolicitationBatchProcessor')
    def test_batch_solicitations_success(self, mock_processor_class, mock_asyncio_run, 
                                       mock_read_csv, runner, tmp_path):
    ...
    def test_command_with_custom_api_key(self, runner):
        ...
        with patch('src.sbir_cet_classifier.cli.enrichment_commands.asyncio.run'):
            with patch('src.sbir_cet_classifier.cli.enrichment_commands.SAMClient') as mock_client:
                result = runner.invoke(app, [
                    "solicitation", "SOL-001", 
                    "--api-key", "custom_key"
                ])
    ...
    def test_batch_command_parameters(self, runner, tmp_path):
        ...
        with patch('src.sbir_cet_classifier.cli.enrichment_commands.pd.read_csv'):
            with patch('src.sbir_cet_classifier.cli.enrichment_commands.asyncio.run'):
                result = runner.invoke(app, [
                    "batch-solicitations", str(test_file),
                    "--batch-size", "5",
                    "--max-concurrent", "2",
                    "--no-skip-existing",
                    "--verbose"
                ...
                assert result.exit_code in [0, 1]  # May fail due to mocking but parameters are valid
